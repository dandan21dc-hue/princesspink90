...
import {
  bulkCreateWorkspaceSlots,
  createWorkspaceSlot,
  deleteWorkspaceSlot,
  listUpcomingWorkspaceSlots,
  updateWorkspaceSlot,
  type WorkspaceSlot,
} from "@/lib/workspace-availability.functions";

export const Route = createFileRoute("/_authenticated/admin/secondary-room-sessions")({
  head: () => ({ meta: [{ title: "Secondary Room Sessions · Admin" }] }),
  component: SecondaryRoomSessionsAdmin,
});
...
function findConflicts(slots: WorkspaceSlot[]): ConflictPair[] {
...
function SecondaryRoomSessionsAdmin() {
  const meFn = useServerFn(amIAdmin);
  const listFn = useServerFn(listUpcomingWorkspaceSlots);
  const createFn = useServerFn(createWorkspaceSlot);
  const updateFn = useServerFn(updateWorkspaceSlot);
  const deleteFn = useServerFn(deleteWorkspaceSlot);
  const bulkFn = useServerFn(bulkCreateWorkspaceSlots);
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Create the windows when the Secondary Room (Glory Holes) is bookable. Customers can only pick times inside an "available" slot. Marking a slot "booked / blocked" hides it from the public picker.
          </p>
        </div>
...
function SlotDialog({
  ...
  initial: WorkspaceSlot | null;
  existingSlots: WorkspaceSlot[];
  pending: boolean;
  onCancel: () => void;
  onSubmit: (v: {
    startTime: string;
